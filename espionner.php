...
else
{
	...
	$req21=$bdd->prepare('SELECT nombre FROM armee WHERE type=? AND joueur=?');
	$req21->execute(array('recrue',$joueur['pseudo']));
	$recrue = $req21->fetch();
	
	$req22=$bdd->prepare('SELECT nombre FROM armee WHERE type=? AND joueur=?');
	$req22->execute(array('capitaine',$joueur['pseudo']));
	$capitaine = $req22->fetch();
	
	$req23=$bdd->prepare('SELECT nombre FROM armee WHERE type=? AND joueur=?');
	$req23->execute(array('lieutenant',$joueur['pseudo']));
	$lieutenant = $req23->fetch();
	
	$req24=$bdd->prepare('SELECT nombre FROM armee WHERE type=? AND joueur=?');
	$req24->execute(array('commandant',$joueur['pseudo']));
	...
	$nombre_unites = $recrue['nombre'] + $capitaine['nombre'] + $lieutenant['nombre'] + $commandant['nombre'];
	...
	if($spy_dif>0)
	{
		$message = '<p class="rap_espio1">Voici les informations qui ont pu être récupérés lors de l\'espionnage de <a href="profil.php?pseudo='.$joueur['pseudo'].'">'.$joueur['pseudo'].'</a> :</p><table class="espiopio"><tr><td class="espio1">Nombre d\'unité(s)</td><td class="espio2">'.number_format($nombre_unites, 0, '.', ' ').'</td></tr>';	
