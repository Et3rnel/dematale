...
if(isset($_POST['subgr']))
{
	...
	if($ressources['bois'] >= $grenier_gold)
	{
		...
		$cout_grenier_f = $cout['grenier'] + $tech['grenier']*75;									
		$niv_grenier_f=$tech['grenier']+1;
		$prod_grenier_f = $capacite['grenier'] + $tech['grenier']*75;
		...
		$req5 = $bdd->prepare('UPDATE niveau SET grenier=? WHERE id=?');			
		$req5->execute(array($niv_grenier_f,$_SESSION['id']));	
